...
- `/banner Заголовок | Текст сообщения` — показать на сайте плашку-объявление (повторный
  вызов заменяет текст)
- `/banner_clear` — убрать плашку-объявление

Все изменения долетают до открытых вкладок сайта мгновенно — без перезагрузки страницы и без
задержки в 30 секунд, через Server-Sent Events (`/api/events`), а не поллинг.
